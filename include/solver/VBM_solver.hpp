...
struct VBM_Node {
  int x, y;
  double f;
  bool operator<(const VBM_Node &other) const { return f > other.f; }
};
...
public:
  ...
  explicit VBM_Solver(Environment &env);
...
private:
  ...
  inline VBM_point coordinatesAt(const size_t index) const {
  ...
  void saveResults(const std::vector<VBM_point> &path) const;
  ...
  std::unique_ptr<VBM_point[]> lightSources_;
  ...
  size_t nx_;
  // Heap openSet_;
  std::unique_ptr<std::priority_queue<VBM_Node>> openSet_;
